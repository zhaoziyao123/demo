        ...
        for (var i = 0; i < options.length; i++) {
            if (options[i].value !== "") {
                if (!uniqueValues.hasOwnProperty(options[i].value)) {
                    selectControl.find(sprintf("option[value=" % s
                    "]", options[i].value
                )).
                    hide();
                } else {
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (column.filterData !== undefined && column.filterData.toLowerCase() !== "column") {
                ...
                if (filterDataType !== null) {
                    ...
                    throw new SyntaxError("Error. You should use any of these allowed filter data methods: var, json, url." + " Use like this: var: {key: "
                    value
                    "}"
                )
                    ;
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            input: function (that, field, isVisible, placeholder) {
                return sprintf("<input type="
                text
                " class="
                form - control
                bootstrap - table - filter - control - % s
                " style="
                width: 100 %;
                visibility: %
                s
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.filterControl && this.options.filterShowClear) {
            ...
            default
                ...
                sprintf("<i class=" % s % s
                "></i> ", this.options.iconsPrefix, this.options.icons.clear
            ),
                "</button>"
            ].
                join("")
    ...
    BootstrapTable.prototype.clearFilterControl = function () {
        if (this.options.filterControl && this.options.filterShowClear) {
            ...
            if (that.options.sortName !== table.data("sortName") || that.options.sortOrder !== table.data("sortOrder")) {
                var sorter = header.find(sprintf("[data-field=" % s
                "]", $(controls[0]).closest("table").data("sortName")
            ))
                ;
                if (sorter.length > 0) {
