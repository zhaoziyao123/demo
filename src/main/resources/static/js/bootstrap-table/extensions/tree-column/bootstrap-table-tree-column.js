    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        onTreeFormatter: function (row) {
            ...
            for (var i = 0; i < plevel; i++) {
                paddings.push("<i class="
                " + options.treeVerticalcls + "
                "></i>"
            )
                ;
                paddings.push("<i class="
                " + options.treeSpacecls + "
                "></i>"
            )
                ;
            }
            ...
            $.each(this.header.fields, function (i, field) {
                if (treeShowField === field) {
                    ...
                    if (that.header.classes[i]) {
                        _class.push(that.header.classes[i].split(""
                        ")[1] || "
                        ");
                    }
                    that.header.classes[i] = "class="
                    " + _class.join("
                    ") + "
                    "";
                    that.header.formatters[i] = function (value, row, index) {
                        var colTree = [that.options.onTreeFormatter.apply(that, [row])];
                        colTree.push("<span class="
                        " + that.options.treeTextcls + "
                        ">"
                    )
                        ;
                        if (_formatter) {
